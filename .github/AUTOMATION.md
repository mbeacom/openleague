...
3. Reads the latest tag with `git describe --tags --abbrev=0`
4. Determines the semantic version bump from commit messages since that tag
5. Skips the remaining steps when the computed tag already exists
6. Creates and pushes an annotated Git tag
7. Creates a GitHub release, with notes generated by GitHub
   (`generate_release_notes`) and announced in the Announcements discussion
8. Notifies success

**Skip Release**: Include `[skip ci]` in commit message

> **Tags are the source of truth for the version — `package.json` is not.**
> The workflow deliberately has no "bump `package.json` and push to `main`"
> step. The `main` ruleset requires four status checks, a direct push from the
> job produces none of them, and the push was rejected with **GH013** — killing
> the job before it reached the tag and release steps that actually matter.
>
> Dropping the bump was safe rather than a workaround: the version is derived
> from `git describe --tags`, and nothing reads `package.json`'s version at
> build or runtime. The field therefore trails the latest tag by design. Read
> the version from the newest tag or the GitHub release — the README version
> badge does.
>
> Restoring the bump means re-breaking releases unless the ruleset gains a
> bypass for the Actions app, which would let any workflow holding
> `contents: write` push to `main` past all four required checks. The full
> rationale lives beside the code it governs, in `release.yml`.
