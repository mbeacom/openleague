...
- Zero [NEEDS CLARIFICATION] markers: the product owner's prior decisions cover the scope-shaping questions (segments as first-class bookable inventory; spatial/visual layout editor in scope; paid public self-service rentals deferred until after feature 007's canonical association reservation work; pre-launch replace-outright for legacy zone fields).
- Design call resolved with the product owner (2026-07-04): hybrid "geometry proposes, declarations decide" — zones are drawn on a surface schematic (presets pre-drawn), drawn geometry suggests coexistence, staff confirm/override, and explicit symmetric declarations remain the stored source of truth for conflict math.
