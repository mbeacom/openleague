/**
 * Read-time standings for season games between real teams — derived from
 * COMPLETED game scores, never stored. Two points for a win, one for a tie;
 * ranked by points, then goal differential, then goals for, then team name.
 *
 * Age gating (FR-030) is enforced by callers when selecting which teams and
 * games to pass in — this module performs no eligibility checks itself.
 */

export type SeasonStandingsGame = {
  status: string;
  homeTeamId: string;
  awayTeamId: string;
